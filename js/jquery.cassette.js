	...
	$.Cassette.prototype 	= {
		...
		_getWheelValues		: function( x ) {
			...
			var T	= this._getSide().current.getDuration(),
				...
				val = {
					left	: ( this.currentSide === 1 ) ? ( -maxTape / T ) * x + maxTape : ( maxTape / T ) * x,
					right	: ( this.currentSide === 1 ) ?  ( maxTape / T ) * x : ( -maxTape / T ) * x + maxTape
				};
			return val;
		...
		_getPosTime			: function() {
			...
			var T		= this._getSide().current.getDuration(),
				posTime	= this.currentSide === 2 ? ( T * wleft ) / maxTape : ( T * wright ) / maxTape;
