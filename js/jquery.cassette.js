...
;(function (window, $, undefined) {
  ...
  $.Cassette.defaults = {
    ...
    songs: ['Peugeot 504 - MASTER V1', 'Esas Cosas - MASTER V1'],
  ...
  $.Cassette.prototype = {
    ...
    _createControls: function () {
      ...
      this.$cSwitch = $(
        '<li class="vc-control-switch">Switch<span></span></li>'
      )
      ...
      this.$volume = $(
        '<div style="display:none;" class="vc-volume-wrap"><div class="vc-volume-control"><div class="vc-volume-knob"></div></div></div> '
      ).appendTo(this.$el)
    ...
    _loadEvents: function () {
      ...
      addTouchSupport(this.$cSwitch, function (event) {
        _self._setButtonActive($(this))
        _self._switchSides()
      })

      addTouchSupport(this.$cPlay, function (event) {
        _self._setButtonActive($(this))
        _self._play()
      })

      addTouchSupport(this.$cStop, function (event) {
        _self._setButtonActive($(this))
        _self._stop()
      })

      addTouchSupport(this.$cForward, function (event) {
        _self._setButtonActive($(this))
        _self._forward()
      })

      addTouchSupport(this.$cRewind, function (event) {
        _self._setButtonActive($(this))
        _self._rewind()
      })
    ...
    _updateStatus: function (buttons, preCalculatedPosTime) {
      var posTime = preCalculatedPosTime || this.cntTime
      ...
      // Only calculate posTime if it wasn't pre-calculated
      if (!preCalculatedPosTime) {
      ...
      // CRITICAL: Update wheel positions immediately within _updateStatus to eliminate timing gap
    ...
    _getWheelValues: function (x) {
      var T = this._getSide().current.getDuration(),
        maxTape = 35 // Increased from 25 to 35 for better visibility

      // Ensure x is within valid bounds to prevent calculation errors
      ...
      // Clamp the calculated values to prevent visual bugs
      val.left = Math.max(0, Math.min(maxTape, val.left))
      val.right = Math.max(0, Math.min(maxTape, val.right))
    ...
    _getPosTime: function () {
      var wleft = this.$wheelLeft.data('wheel'),
        ...
        maxTape = 35 // Updated to match the value used in _getWheelValues
    ...
    _updateWheelValue: function (wheelVal) {
      this.$wheelLeft
        .data('wheel', wheelVal.left)
        .css({ 'box-shadow': '0 0 0 ' + wheelVal.left + 'px black' })
      this.$wheelRight
        .data('wheel', wheelVal.right)
        .css({ 'box-shadow': '0 0 0 ' + wheelVal.right + 'px black' })
    },
  ...
  $.fn.cassette = function (options) {
    if (typeof options === 'string') {
      ...
      this.each(function () {
        ...
        if (!instance) {
          logError(
            'cannot call methods on cassette prior to initialization; ' +
